...
变量：
	var num1,mun2 ; //声明一个变量num1，num2,变量也可以不声明，直接使用.
	错误:
	6num  //开头不能用数字
	...
	'+'算术运算符主要用来完成类似加减乘除的工作，在JavaScript中，“+”不只代表加法，还可以连接两个字符串，
		例如：mystring = "Java" + "Script"; // mystring的值“JavaScript”这个字符串
...
创建数组语法：
		...
		var myarray= new Array(8); //创建数组，存储8个数据。指定长度,但实际上数组都是变长的，也就是说即使指定了长度为8，仍然可以将元素存储在规定长度以外
	数组赋值：
		第一种方法：
	...
	数组属性length
		...
		JavaScript数组的length属性是可变的：	arr.length=10; //增大数组的长度
二维数组：	myarray[ ][ ]	说明: myarr[0][1] ,0 表示表的行，1表示表的列。
	二维数组的定义方法二
	...
	JavaScript 中的所有事物都是对象，如:字符串、数值、数组、函数等，每个对象带有属性和方法。
		...
		Date 日期对象: var Udate=new Date(); //有初始值：当前时间(当前电脑系统时间)。
	...
	Math对象，提供对数据的数学计算
		Math.floor(x); Math.ceil(x) ;Math.round(x)	//向下，向上取整 ,四舍五入
		Math.random(); Math.round(x)	//随机数; 取整
